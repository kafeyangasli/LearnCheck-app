...
export const getTutorialContent = async (tutorialId: string): Promise<string> => {
  try {
    logger.info(`[Dicoding API] Fetching tutorial content for ID: ${tutorialId}`);
    ...
    logger.error('[Dicoding API] Error fetching tutorial content:', error.message);
    if (error.response) {
...
export const getUserPreferences = async (userId: string): Promise<any> => {
  try {
    ...
    logger.error('[Dicoding API] Error fetching user preferences:', error.message);
    if (error.response) {
      logger.error(`[Dicoding API] Response status: ${error.response.status}`);
    }
